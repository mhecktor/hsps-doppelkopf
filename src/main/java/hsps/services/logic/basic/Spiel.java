...
import hsps.services.logic.rules.announcements.KeineNeunzig;
...
public class Spiel extends AbstractRoundBasedGame {
	...
	public boolean contraCalled = false;
	...
	public Spiel( String spielID ) {
		...
		this.decisionAnnouncements.add( new KeineNeunzig() );
	...
	protected void preTurn() {
		validCard = true;
		currentDecisionAnnouncement = null;
		for( int i = 0; i < decisionAnnouncements.size() && currentDecisionAnnouncement == null; i++ ) {
			if( decisionAnnouncements.get( i ).test( this ) ) {
				decisionAnnouncements.get( i ).inform();
				currentDecisionAnnouncement = decisionAnnouncements.get( i );
			}
		}
	}
	...
	private void auswerten() {
		...
		if( punkteContra < 120 ) siegRe = true;
		...
		for( Spieler s : spielerListe ) {
			if( s.isRe() ) {
				if( siegRe ) {
					...
					for( DecisionAnnouncement decisionAnnouncement : decisionAnnouncements ) {
						if( decisionAnnouncement.called ) s.getStatistik().setPunkte( s.getStatistik().getPunkte() + 1 );
					}
			...
			if( DEBUG ) System.out.println( s.getName() + "\n\t" + s.getStatistik() );
		}
	...
	private void kartenTestMode() {
		...
		while( kartenAnzahl > 0 ) {
			...
			if( k.getSymbolik() == Symbolik.KOENIG && spielerListe[ 0 ].getHand().getKarten().size() < 10 ) {
				...
				MqttService.publisher.publishData( new Message( MessageType.GetCard, k ), Topic.genPlayerTopic( getSpielID(), spielerListe[ 0 ].getName() ) );
